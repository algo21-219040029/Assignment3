...
class BilateralTradeHoldFactor1(BaseTradeHoldFactor):
    """
    双边成交持仓因子1: (sum(持多仓量,N)-sum(持空仓量,N))/品种持仓量
    ...
    def compute_factor(self) -> DataFrame:
        ...
        common_index = long_df.index.intersection(short_df.index).intersection(open_interest_df.index)
        common_columns = long_df.columns.intersection(short_df.columns).intersection(open_interest_df.columns)
        ...
        # 计算原始因子值
        factor_value: DataFrame = (long_df - short_df) / open_interest_df
...
if __name__ == "__main__":
    self = BilateralTradeHoldFactor1()
