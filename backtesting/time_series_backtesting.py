...
class TimeSeriesBacktesting(BaseBacktesting):
    ...
    def run_backtesting(self, tqdm_flag: bool = True) -> None:
        ...
        rolling_profit_series_dict = {}
        rolling_long_profit_series_dict = {}
        rolling_short_profit_series_dict = {}
        rolling_cumsum_profit_series_dict = {}
        rolling_long_cumsum_profit_series_dict = {}
        rolling_short_cumsum_profit_series_dict = {}

        rolling_profit_df_dict = {}
        rolling_long_profit_df_dict = {}
        rolling_short_profit_df_dict = {}
        ...
        for shift in range(period):
            ...
            rolling_profit_series_dict[shift] = profit_df.fillna(0.0).sum(axis=1)
            rolling_long_profit_series_dict[shift] = long_profit_df.fillna(0.0).sum(axis=1)
            rolling_short_profit_series_dict[shift] = short_profit_df.fillna(0.0).sum(axis=1)
            ...
            rolling_profit_df_dict[shift] = profit_df.fillna(0.0)
            rolling_long_profit_df_dict[shift] = long_profit_df.fillna(0.0)
            rolling_short_profit_df_dict[shift] = short_profit_df.fillna(0.0)
        ...
        rolled_profit_series: Series = pd.Series()
        rolled_long_profit_series: Series = pd.Series()
        rolled_short_profit_series: Series = pd.Series()
        ...
        rolled_profit_df: DataFrame = pd.DataFrame()
        rolled_long_profit_df: DataFrame = pd.DataFrame()
        rolled_short_profit_df: DataFrame = pd.DataFrame()
        ...
        for shift in rolling_profit_series_dict:
            if shift == 0:
                rolled_profit_series = rolling_profit_series_dict[shift]
                rolled_long_profit_series = rolling_long_profit_series_dict[shift]
                rolled_short_profit_series = rolling_short_profit_series_dict[shift]
                ...
                rolled_profit_df = rolling_profit_df_dict[shift]
                rolled_long_profit_df = rolling_long_profit_df_dict[shift]
                rolled_short_profit_df = rolling_short_profit_df_dict[shift]
        ...
        rolled_profit_series = rolled_profit_series / len(rolling_profit_series_dict)
        rolled_long_profit_series = rolled_long_profit_series / len(rolling_long_profit_series_dict)
        rolled_short_profit_series = rolled_short_profit_series / len(rolling_short_profit_series_dict)
        ...
        rolled_profit_df = rolled_profit_df / len(rolling_profit_df_dict)
        rolled_long_profit_df = rolled_long_profit_df / len(rolling_long_profit_df_dict)
        rolled_short_profit_df = rolled_short_profit_df / len(rolling_short_profit_df_dict)
        ...
        ## 构建行业资金曲线
        # 初始化变量
        industry_rolled_profit_series: Dict[str, Series] = {}
        industry_rolled_long_profit_series: Dict[str, Series] = {}
        industry_rolled_short_profit_series: Dict[str, Series] = {}
        ...
        industry_rolled_profit_df: Dict[str, DataFrame] = {}
        industry_rolled_long_profit_df: Dict[str, DataFrame] = {}
        industry_rolled_short_profit_df: Dict[str, DataFrame] = {}
        ...
        for industry in industry_list:
            ...
            industry_rolled_profit_series[industry] = rolled_profit_df[symbol_list].sum(axis=1)
            industry_rolled_long_profit_series[industry] = rolled_long_profit_df[symbol_list].sum(axis=1)
            industry_rolled_short_profit_series[industry] = rolled_short_profit_df[symbol_list].sum(axis=1)
            ...
            industry_rolled_profit_df[industry] = rolled_profit_df[symbol_list]
            industry_rolled_long_profit_df[industry] = rolled_long_profit_df[symbol_list]
            industry_rolled_short_profit_df[industry] = rolled_short_profit_df[symbol_list]
        ...
        self.backtest_curve['industry']['cumsum_profit'] = industry_rolled_cumsum_profit_series
        self.backtest_curve['industry']['long_cumsum_profit'] = industry_rolled_long_cumsum_profit_series
        self.backtest_curve['industry']['short_cumsum_profit'] = industry_rolled_short_cumsum_profit_series
